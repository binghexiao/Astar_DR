...
#pragma region case1: 新点
        ...
        if (m.op.empty() && m.cl.empty()) {
            ...
            {
                Util::cost_check_cnt += COSTS.size();
                continue;
            }
            // 放入OPEN列表
...
#pragma region case3：新的g
        ...
        // g_m不被op与cl中其他向量支配
            ...
            Util::cl_check_cnt = Util::cl_check_cnt + m.op.size() + m.cl.size();
...
void Multi_Astar::NextStep2(Point& current, array_type current_g, H3Index goal, int& Size, Parameter& parameter)
{
    ...
    for (auto it = current.neighbours.begin(); it != current.neighbours.end(); it++) {
        ...
        //  情况2：被g支配
        if ( NodeMap[m.getIndex()].g_2_min <= g_m[1])
        {
            // cout << "进来了" << endl;
            Util::cl_check_cnt++;
            continue;
        }else if(m.op.dominates(point_g_m))
        {
            Util::op_check_cnt += 2 * m.op.size();
            continue;
        }
